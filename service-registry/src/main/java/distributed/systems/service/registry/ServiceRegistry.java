package distributed.systems.service.registry;
...
public class ServiceRegistry implements Watcher {
    ...
    public void registerToCluster(String metadata) throws KeeperException, InterruptedException {
        if(currentZnodeName != null)
                return;
        currentZnodeName = zooKeeper.create(REGISTRY_NAMESPACE+"/n_",metadata.getBytes(),ZooDefs.Ids.OPEN_ACL_UNSAFE,CreateMode.EPHEMERAL_SEQUENTIAL);
        System.out.println("Registered to service registry as " + (currentZnodeName.replace(REGISTRY_NAMESPACE+"/","")));
    }
    ...
    private synchronized  void updateAddresses() throws KeeperException, InterruptedException {
        ...
        for(String workZnode : workerZnodes) {
            String workerZnodeFullPath = REGISTRY_NAMESPACE + "/" + workZnode;
